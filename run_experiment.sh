...
serial_or_parallel=${6?Input serial_or_parallel}

domino_to_chipmunk $domino_program > /tmp/tmp.sk

cd $chipmunk_folder
Output_result=$(python3 chipmunk.py /tmp/tmp.sk $alu $pipeline_len $pipeline_width codegen sample1 $serial_or_parallel)
